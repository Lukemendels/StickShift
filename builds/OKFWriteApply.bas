...
Sub ApplyOKFWrite()
    ...
    Do
        ...
        If eolPos = 0 Then Exit Do                              ' malformed — no newline after path
        ...
        If endFilePos = 0 Then Exit Do                         ' malformed — no END FILE marker

        Dim contents As String
        contents = Mid(body, eolPos + 1, endFilePos - eolPos - 1)
        ' Strip one trailing newline that separates contents from ### END FILE.
    ...
    For i = 0 To fileCount - 1
        ...
        ' Ensure parent directory exists (e.g. builds/ may not yet exist).
        ...
        If Not fso.FileExists(absPath) Then
            ...
            newCount = newCount + 1
        Else
            ' Existing file — stage as .proposed; never overwrite.
...
Private Function ResolvePath(ByVal relPath As String) As String
    ...
    p = Trim(relPath)
    p = Replace(p, "/", "\")
    If Left(p, 1) = "\" Then p = Mid(p, 2)    ' strip leading backslash
